...
var SuperGif = function ( options ) {
	...
	var handler = {
		...
		eof: function (block) {
			...
			console.log("As Brush:"+importAsBrush);
            window.pjsin.setupImportedGif(gifFrames[0].data.width,gifFrames[0].data.height,gifFrames.length,importAsBrush);
			for (var i=0; i < gifFrames.length; i++) {
				window.pjsin.importFrame(gifFrames[i].data,i,importAsBrush);
			}
			if (!importAsBrush) {
	...
	return {
		load: function (callback,gifByteStream,importAsBrushBool) {
			loading = true;
			console.log("as brush bool:"+importAsBrushBool);
			importAsBrush = importAsBrushBool;
			init();
