...
//       :: :: :  :    :        :        :   : : :   ::    :     :   
//     
//             ~ BRUSHSHOCK!   FILLSHOCK!   INKDROPPERSHOCK! ~
//                ~ NOW IS TIME TO THE 89a HEART ON FIRE! ~
//

  window.pjsin = Processing.getInstanceById("paintCanvas");
    ...
    $('#paintCanvas').css('image-rendering','pixelated');
    ...
    void addFrame(){
        ...
        //console.log("Frames after addition :"+frames.length);
            ...
            tracingFramesBefore[tracingFramesBefore.length-1].image(tracingFramesBefore[tracingFramesBefore.length-2]);
	...
	void setTool(int tool){
		switch(tool){
			...
			case 3:
				//console.log("Camera");
				mouseClickTool = webcam;
				mouseDragTool  = nullTool;
			break;
			case 4:
				console.log("GifBrush");
				mouseClickTool = gifBrush;
				mouseDragTool  = gifBrush;
			break;
		default:
			mouseClickTool = pencilLine;
			mouseDragTool  = pencilLine;
    ...
    void useTool(lasyX,lastY,curX,curY,button){ mouseDragTool(lasyX,lastY,curX,curY,button);  }
    ...
    // // [
    // if (key == 91) { $('#tool_onion').click(); };
    // // ]
    // if (key == 93) { $('#tool_onionNext').click(); };
    //  // r
	...
	void gotoFrame(framenum){
		...
		if (onionSkinNext) {
            tint(255, 50);
            image(frames[getOnionNextFrame(framenum)],0,0);
    		noTint();		    
		};
		image(frames[framenum],0,0);
		currentFrame = framenum;
        $("#currentFrame").text(parseInt(currentFrame)+1);
...
//    
    ...
    void webcam(int x,int y,int x2, int y2, int mouseBut){
		window.snap();
    };
    
    void swapColours(){
        var swapwell = $('#leftColour').val()
        $('#leftColour').miniColors('value',$('#rightColour').val());				    
        $('#rightColour').miniColors('value',swapwell);				    
    }
...
void importImg(uri){
	//console.log("Importing Image:"+uri);
...
var firstFrame = true;
var gifBrushLoaded = false;
void setupImportedGif(newGifWidth,newGifHeight,newGifLength,importAsBrush){
	if (importAsBrush) {
		firstFrame = true;
		gifBrushLoaded = true;
		initGifBrush(newGifWidth,newGifHeight,newGifLength);
...
void importFrame(frameData,frameNumber,importAsBrush){
	if (importAsBrush) {		
		if (firstFrame) {
			// Draw Preview
			$('#gifBrushPreview').removeClass("instruction");
			var gbpc = document.getElementById('gifBrushPreviewCanvas');
			var gbpcContext = gbpc.getContext("2d");
			gbpcContext.fillStyle = "#FFFFFF";
			gbpcContext.fillRect ( 0, 0, 70, 70);
			gbpcContext.putImageData(frameData,33+(frameData.width/2)*-1,33+(frameData.height/2)*-1);
			firstFrame = false;
		}
		console.log("Trying brush:"+frameNumber);
		importedFrame.fromImageData(frameData);
		addGifBrushFrame(frameNumber,importedFrame);
	} else {
		console.log("Trying:"+frameNumber);
	    importedFrame.fromImageData(frameData);
